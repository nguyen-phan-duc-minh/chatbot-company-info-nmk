...
You: Các dự án biệt thự của NMK?
Bot: Chào bạn! 👋 NMK có những dự án biệt thự này nè:
...
curl -X POST http://localhost:8000/api/chat \
  ...
  -d '{"query": "Phong cách nội thất của NMK?"}'
```
...
1. **Basic Query**
   ```
   You: Các dự án của NMK?
   → Expect: List projects
   ```

2. **Specific Query**
   ```
   You: Biệt thự hiện đại quận 3?
